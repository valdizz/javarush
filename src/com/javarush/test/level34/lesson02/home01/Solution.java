...
public class Solution {
    ...
    private void customPrint(DecimalFormat df, double v, int countOperation, int currentCountOperation) {
        if (countOperation == 0) {
            System.out.println(df.format(new BigDecimal(v).setScale(2, RoundingMode.HALF_UP)) + " " + currentCountOperation);
        } else {
            System.out.println(new BigDecimal(v).setScale(4, RoundingMode.HALF_UP).toString() + " " + currentCountOperation);
        }
    }
