...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> classLoader, Class<?>... interfaces){
        Class[] _interfaces = new Class[interfaces.length + 1];
        System.arraycopy(interfaces, 0, _interfaces, 0, interfaces.length);
        _interfaces[_interfaces.length - 1] = classLoader;
        return (T) Proxy.newProxyInstance(classLoader.getClassLoader(), _interfaces, new ItemInvocationHandler());
    }
}
